...
public class CommonUtil {
     ...
     * @param instList
     */
    public static void fillBackRelationInfo(List<Map<String,Object>> instList, Object daoInterface, String relationKey, String[] args) throws Exception {
        ...
        for (Map<String, Object> inst : instList) {
            keySet.add(MapUtils.getLongValue(inst, relationKey));
        }
        List<Long> keyIds = new ArrayList<>();
        keyIds.addAll(keySet);

        String methodName = getMethodNameByRelationKey(relationKey);
        if (StringUtils.isBlank(methodName) || args.length < 1) {
            return;
        }

        Method method = daoInterface.getClass().getDeclaredMethod(methodName, List.class);
        Object resObj = method.invoke(daoInterface, keyIds);

        // 生key与map的映射关系
        List<Map<String, Object>> list = (List<Map<String, Object>>) resObj;
        Map<Long, Map<String, Object>> keyIdMapping = new HashMap<>();
        for (Map<String, Object> map : list) {
            keyIdMapping.put(MapUtils.getLongValue(map, relationKey), map);
        }

        for (Map<String, Object> inst : instList) {
            Long keyValue = MapUtils.getLongValue(inst, relationKey);
            Map<String, Object> attrValueMap = keyIdMapping.get(keyValue);
            if (attrValueMap != null) {	//防止脏数据
                for (String arg : args) {
                    inst.put(arg, attrValueMap.get(arg));
                }
            }
        }
    }
